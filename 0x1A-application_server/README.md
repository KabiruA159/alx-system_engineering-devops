...
Configuration Steps: 
The text outlines a sequence of tasks that involve configuring the deployment environment, setting up Nginx, Gunicorn, and Flask to serve the web application.

Tasks Details 📃
0. Set up development with Python
Setting up the development environment by configuring a Flask app to serve content on a specific route and port.
...
Nginx configuration file proxying requests on the route /airbnb-onepage/ to the Gunicorn app running on port 5000.
3. Add a route with query parameters

3-app_server-nginx_config: 
Nginx configuration file proxying requests on the route /airbnb-dynamic/number_odd_or_even/<int: num> to the Gunicorn app running on port 5000.
